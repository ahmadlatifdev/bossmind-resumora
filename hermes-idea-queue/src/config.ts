...
const envSchema = z.object({
  ...
  MCP_PORT: z.coerce.number().default(8791),
  HITL_TOKEN: z.string().default('change-me-local-only'),
  /** Public Resumora API used to PATCH project status (not the local HITL port). */
  RESUMORA_API_BASE: z.string().default('https://resumora.net'),
  HERMES_API_URL: z.string().optional().default('http://127.0.0.1:8790'),
  ADMIN_REFUND_PASSWORD: z.string().optional().default(''),
  VITE_ADMIN_PASSWORD: z.string().optional().default(''),
  AUTO_RECOVERY_INTERVAL_MS: z.coerce.number().default(10_000),
