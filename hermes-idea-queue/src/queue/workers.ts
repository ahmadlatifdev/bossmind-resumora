...
async function handleJob(payload: QueueJobPayload): Promise<void> {
...
export function startWorkers(): { taskWorker: Worker; qaWorker: Worker } {
  const cfg = loadConfig();
  const taskWorker = new Worker<QueueJobPayload>(QUEUE_NAME, async (job) => handleJob(job.data), {
    connection: bullConnection(),
    concurrency: cfg.MAX_CONCURRENT_TASKS,
  });
  const qaWorker = new Worker<QueueJobPayload>(
    QA_QUEUE_NAME,
