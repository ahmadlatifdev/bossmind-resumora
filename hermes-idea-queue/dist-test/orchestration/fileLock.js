...
const lockKey = (filePath) => `idea-queue:filelock:${filePath.replace(/\\/g, '/')}`;
export async function acquireFileLocks(files, owner) {
...
export async function releaseFileLocks(files, owner) {
  const redis = getRedis();
...
export async function refreshFileLocks(files, owner) {
  const redis = getRedis();
  const ttl = loadConfig().FILE_LOCK_TTL_SEC;
  for (const file of files) {
    const key = lockKey(file);
