...
export async function createFeatureBranch(ideaId) {
...
export async function atomicCommitAndPush(branch, message) {
  const cfg = loadConfig();
  const cwd = cfg.REPO_ROOT;
  await git(['add', '-A'], cwd);
...
export async function openPullRequest(branch, title, body) {
  const cfg = loadConfig();
  if (!cfg.GITHUB_TOKEN) {
