...
- **BossMind AI Video Generator (isolated `ai-video-generator` project_key):** **`config/bossmind-ai-video-stack.json`**, **`/bossmind-ai-video`**, **`/api/orchestration/ai-video/*`**, **`npm run bossmind:ai-video:ensure-tables`**. Secrets: **`BOSSMIND_AI_VIDEO_ADMIN_SECRET`**, **`BOSSMIND_AI_VIDEO_N8N_SECRET`** (see `.env.example`).
