...
- **BossMind rolling backup (30d) + multi-project hub orchestrator:** **`npm run bossmind:backup:daily`** (single repo), **`npm run bossmind:backup:multi`** (set optional secret **`BOSSMIND_BOSSMIND_ROOT`** on self-hosted runner or locally for `D:\\BossMind` siblings), **`npm run bossmind:backup:simulate`**, **`npm run bossmind:backup:production-report`**, Neon audit **`npm run bossmind:backup:architecture-lock -- --i-understand-external-hub`**. Health JSON includes **`backupPreservation`**. Full DB dumps / Render env exports stay external.
