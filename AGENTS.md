...
- **Resumora AI support mail (Gmail + n8n + Neon policy):** **`docs/RESUMORA_AI_SUPPORT_MAIL_AUTOMATION.md`** + **`config/resumora-ai-support-mail-architecture.json`** + **`npm run resumora:support:ai:arch-lock`** (Neon audit after external go-live). **DNS + dedupe API + Neon verification lock:** **`npm run resumora:support:mail:verify`** + **`POST /api/orchestration/support-mail-dedupe`** (Bearer `BOSSMIND_SUPPORT_WEBHOOK_SECRET`).
...
- **BossMind rolling backup (30d) + multi-project hub orchestrator:** **`npm run bossmind:backup:daily`** (single repo), **`npm run bossmind:backup:multi`**, **`npm run bossmind:backup:activate-full`** (single-shot: multi + per-project validate + **strict** restore simulate + production report + optional Neon lock when **`BOSSMIND_BACKUP_CONFIRM_PRODUCTION_LOCK=1`**). Set optional secret **`BOSSMIND_BOSSMIND_ROOT`** for hub siblings. **`.env` / secrets paths are never copied** (Anti-Leak filter in `bossmind-backup-daily`). Health JSON **`backupPreservation`**. Neon dumps / Render env exports stay external.
