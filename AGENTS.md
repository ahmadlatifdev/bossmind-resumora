...
- **Google marketing ecosystem audit (live site + DNS + optional PageSpeed API; not GSC/GA4/Ads dashboards):** **`npm run resumora:google:ecosystem:audit`**, **`GET /api/orchestration/bossmind-health`** (`googleEcosystem.lastReport`). Neon lock of latest JSON report: **`npm run resumora:google:ecosystem:arch-lock -- --i-understand-dashboard-gaps --notes="..."`**. Optional **`GOOGLE_PAGESPEED_API_KEY`**.
- **Google Traffic Engine (GSC recovery + GA4 validate + DeepSeek/OpenAI/Gemini/SE Ranking layers + weekly Mon–Sun schedule):** **`config/resumora-google-traffic-engine.json`**, **`npm run resumora:google-traffic:optimize`**, **`npm run resumora:ga4:validate`**, **`npm run resumora:gsc:verify-recovery`**, Neon lock **`npm run resumora:google-traffic:lock -- --i-understand-traffic-config`**. Health: **`googleTrafficEngine.lastReport`** on **`GET /api/orchestration/bossmind-health`**.
- **Ultra Anti-Leak (98% target, proof-based scoring):** **`config/bossmind-ultra-antileak-policy.json`**, **`npm run bossmind:ultra:antileak`**, snapshot+lock **`npm run bossmind:ultra:antileak:snapshot-lock`**, restore **`npm run bossmind:ultra:antileak:restore`**, self-heal **`npm run bossmind:ultra:antileak:self-heal`**, closed-loop **`npm run bossmind:ultra:antileak:closed-loop`**, deploy gate hook **`BOSSMIND_DEPLOY_GATE_ULTRA=1`**, pricing UI **`npm run resumora:pricing:ui-verify`**. Health: **`ultraAntileak`** on **`GET /api/orchestration/bossmind-health`**. Authority markers in **`config/bossmind-protected-ui-authority.json`** (no `id="trust"`; requires Essential Advanced + pricing UI marker).
- **GSC ownership DNS recovery (TXT scan, global propagation, conflict report, optional API probe, Neon lock):** **`npm run resumora:gsc:verify-recovery`**, **`config/resumora-gsc-verification-authority.json`**, lock with **`--lock`**. Preserves Workspace SPF/DKIM/DMARC; does not mutate DNS at registrar.
- **BossMind AI Video — channel VibeVoyage (isolated `ai-video-generator` project_key):** **`config/bossmind-ai-video-stack.json`**, **`config/vibevoyage-brand.json`**, **`/bossmind-ai-video`**, **`/api/orchestration/ai-video/*`**, **`npm run bossmind:ai-video:ensure-tables`**. Optional **`BOSSMIND_AI_VIDEO_CHANNEL_NAME`**, **`BOSSMIND_AI_VIDEO_TAGLINE`**, **`BOSSMIND_AI_VIDEO_MISSION`**. Secrets: **`BOSSMIND_AI_VIDEO_ADMIN_SECRET`**, **`BOSSMIND_AI_VIDEO_N8N_SECRET`** (see `.env.example`).
