...
function flagFor(code: string): string {
  return [...code.toUpperCase()]
    .map((letter) => String.fromCodePoint(letter.charCodeAt(0) + FLAG_OFFSET))
    .join('');
}

export const COUNTRIES: Country[] = RAW_COUNTRIES.split('\n')
  .map((line) => {
    const [code, name, dial] = line.split('|');
    return { code, name, dial, flag: flagFor(code) };
  })
  .sort((a, b) => a.name.localeCompare(b.name));

export function detectDefaultCountry(): Country {
  const locale = typeof navigator !== 'undefined' ? navigator.language || '' : '';
  const region = locale.match(/[-_]([A-Za-z]{2}|\d{3})$/)?.[1]?.toUpperCase();
  return (
...
export function formatE164(dial: string, local: string): string {
