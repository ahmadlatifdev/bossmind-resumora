...
const BASE_URL =
  (process.env.PLAYWRIGHT_BASE_URL || 'http://127.0.0.1:5173').replace(/\/$/, '') +
  '/admin/master#hermes-chat';
...
async function selectProject(page, name) {
  ...
  if (labels.some((t) => t.trim() === name || t.includes(name))) {
    await select.selectOption({ label: name });
    return;
  }
  // Local harness may only list Resumora — inject option so per-project isolation still runs.
  await select.evaluate((el, label) => {
    const value = label.toLowerCase().replace(/\s+/g, '-');
    let opt = Array.from(el.options).find((o) => o.textContent === label);
    if (!opt) {
