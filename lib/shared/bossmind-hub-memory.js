...
const { repairBossmindHubSchema } = require("./bossmind-hub-schema-repair");
...
const BOSSMIND_HUB_TABLES_SQL = [
  ...
  `CREATE TABLE IF NOT EXISTS bossmind_payment_links (
    id BIGSERIAL PRIMARY KEY,
    project_key TEXT NOT NULL,
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (project_key, service_key)
  )`,
  `CREATE INDEX IF NOT EXISTS idx_bossmind_payment_links_project ON bossmind_payment_links(project_key, updated_at DESC)`,
...
async function initializeBossmindHubMemory() {
  ...
  await repairBossmindHubSchema().catch(() => {});
...
async function upsertPaymentLink({
  ...
  if (!sql) return null;
  const rows = await sql(
    `INSERT INTO bossmind_payment_links
     (project_key, service_key, product_id, price_id, payment_link_id, payment_link_url, amount_cents, currency, metadata, locked, updated_at)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9::jsonb, $10, NOW())
     ON CONFLICT (project_key, service_key) DO UPDATE SET
       ...
       locked = EXCLUDED.locked,
       updated_at = NOW()
     RETURNING id, service_key, payment_link_url, updated_at`,
    [
...
async function listPaymentLinks({ projectKey, limit = 30 } = {}) {
  const sql = getSqlClient();
  if (!sql) return [];
  const rows = await sql(
    `SELECT service_key, product_id, price_id, payment_link_id, payment_link_url, amount_cents, currency, metadata, locked, updated_at
     FROM bossmind_payment_links WHERE project_key = $1 ORDER BY updated_at DESC LIMIT $2`,
    [normalizeProjectKey(projectKey), limit]
  );
  return rows || [];
}
