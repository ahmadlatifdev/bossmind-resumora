...
const REQUIRED_TABLES_SQL = [
  ...
  `CREATE INDEX IF NOT EXISTS idx_client_entitlements_session ON client_entitlements(stripe_session_id)`,
  `CREATE TABLE IF NOT EXISTS client_checkout_activation (
    id BIGSERIAL PRIMARY KEY,
  ...
  `CREATE INDEX IF NOT EXISTS idx_client_checkout_activation_phase ON client_checkout_activation(phase)`,
  ...
  `CREATE TABLE IF NOT EXISTS client_workspace_documents (
    id BIGSERIAL PRIMARY KEY,
    profile_id UUID NOT NULL REFERENCES engagement_profiles(id) ON DELETE CASCADE,
    plan_id TEXT NOT NULL,
  ...
  `CREATE INDEX IF NOT EXISTS idx_client_workspace_documents_profile ON client_workspace_documents(profile_id, created_at DESC)`,
  `CREATE INDEX IF NOT EXISTS idx_client_workspace_documents_plan ON client_workspace_documents(plan_id, created_at DESC)`,
  `CREATE UNIQUE INDEX IF NOT EXISTS ux_client_workspace_documents_active_path
    ON client_workspace_documents(profile_id, storage_path)
    WHERE removed_at IS NULL`,
  `CREATE TABLE IF NOT EXISTS client_delivery_status (
    id BIGSERIAL PRIMARY KEY,
    profile_id UUID NOT NULL REFERENCES engagement_profiles(id) ON DELETE CASCADE,
    plan_id TEXT NOT NULL,
  ...
  `CREATE INDEX IF NOT EXISTS idx_client_edit_requests_profile ON client_edit_requests(profile_id, requested_at DESC)`,
...
/** Legacy Neon tables (pre-2026) used different column names — rename before CREATE IF NOT EXISTS. */
const LEGACY_TABLE_MIGRATIONS = [
  {
    table: "event_log",
    markerColumn: "project_key",
    legacySuffix: "legacy_v0",
  },
  {
    table: "task_state",
    markerColumn: "project_key",
...
async function migrateLegacyBossmindTables(sql) {
  const actions = [];
  for (const { table, markerColumn, legacySuffix } of LEGACY_TABLE_MIGRATIONS) {
    ...
    if (columns.includes(markerColumn)) continue;
    const legacyName = `${table}_${legacySuffix}`;
    const legacyExists = await sql.query(
      `SELECT 1 FROM information_schema.tables
       WHERE table_schema = 'public' AND table_name = $1 LIMIT 1`,
      [legacyName]
    );
    if (legacyExists?.length) continue;
    await sql.query(`ALTER TABLE ${table} RENAME TO ${legacyName}`);
    actions.push({ table, renamedTo: legacyName, previousColumns: columns });
  }
  return actions;
}
...
async function initializeSharedMemory() {
  ...
  try {
    legacyMigrations = await migrateLegacyBossmindTables(sql);
  } catch (e) {
    return { enabled: false, reason: e.message || "legacy_schema_migration_failed" };
  }
  ...
  return { enabled: true, hub, warnings, legacyMigrations };
}
