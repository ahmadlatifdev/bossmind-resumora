...
/** Auth, entitlements, prep progress, and checkout — isolated from legacy BossMind DDL drift. */
const ENGAGEMENT_SCHEMA_SQL = REQUIRED_TABLES_SQL.slice(
  REQUIRED_TABLES_SQL.findIndex((s) => s.includes("engagement_profiles")),
  REQUIRED_TABLES_SQL.findIndex((s) => s.includes("idx_client_prep_progress_profile")) + 1
);
...
function getDatabaseConfigStatus() {
  ...
  return {
    ...
    checkedKeys: ["NEON_DATABASE_URL", "DATABASE_URL", "POSTGRES_URL", "POSTGRES_PRISMA_URL"],
  };
}
...
async function ensureEngagementSchema() {
  ...
  if (!engagementSchemaInitPromise) {
