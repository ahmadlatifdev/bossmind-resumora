...
async function recordSocialClick(profileId, visitorId, platform, href, regionHint, source = "footer_social_strip") {
  const safePlatform = String(platform || "").toLowerCase().slice(0, 32);
  if (!safePlatform) return { ok: false, error: "missing_platform" };
  await logActivity({
    profileId,
    visitorId,
    resourceKey: `social:${safePlatform}`,
    action: "social_click",
    regionHint,
  });
  try {
    await saveEvent({
      projectKey: "resumora",
      eventType: "engagement_social_click",
      severity: "info",
      source: `social.${safePlatform}`,
      eventKey: safePlatform,
      payload: {
        platform: safePlatform,
        href: String(href || "").slice(0, 512),
        source,
...
async function getAggregateStats() {
  ...
  const dislikesByResource = await sql(
    `SELECT resource_key AS key, COUNT(*)::int AS count FROM engagement_dislikes GROUP BY resource_key ORDER BY count DESC`
  );
  ...
  const sharesTotal = await sql(
    `SELECT COUNT(*)::int AS c FROM engagement_activity WHERE action = 'share'`
  );
  const socialClicks = await sql(
    `SELECT REPLACE(resource_key, 'social:', '') AS key, COUNT(*)::int AS count
     FROM engagement_activity
     WHERE action = 'social_click' AND resource_key LIKE 'social:%'
     GROUP BY resource_key
     ORDER BY count DESC`
  );
  const configureSignals = await sql(
    `SELECT resource_key AS key, COUNT(*)::int AS count
     FROM engagement_activity
     WHERE action = 'configure_open'
     GROUP BY resource_key
     ORDER BY count DESC`
  );

  const weighted = {};
  for (const row of likesByResource) weighted[row.key] = (weighted[row.key] || 0) + Number(row.count || 0) * 1;
  for (const row of savesByResource) weighted[row.key] = (weighted[row.key] || 0) + Number(row.count || 0) * 1.2;
  for (const row of requestsByResource) weighted[row.key] = (weighted[row.key] || 0) + Number(row.count || 0) * 2.2;
  for (const row of configureSignals) weighted[row.key] = (weighted[row.key] || 0) + Number(row.count || 0) * 0.8;
  const trendingServices = Object.entries(weighted)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([key, score]) => ({ key, score: Number(score.toFixed(2)) }));

  const topSignal = trendingServices[0]?.score || 0;
  const conversionScoring = trendingServices.map((row) => ({
    ...row,
    heat: topSignal > 0 ? Number(((row.score / topSignal) * 100).toFixed(1)) : 0,
  }));
  ...
  return {
    ...
    savesByResource,
    requestsByResource,
    configureByResource: configureSignals,
    socialClicksByPlatform: socialClicks,
    sharesTotal: sharesTotal[0]?.c ?? 0,
    trendingServices,
    conversionScoring,
    regional,
  };
