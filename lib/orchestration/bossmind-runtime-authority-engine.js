...
function crossAiEnforcement(changeDescriptor, contextPayload) {
  ...
  const agents = {
    ...
    windsurf: { role: "review_only", allowed: true },
  };
...
async function detectDeployDrift({ cwd, projectKey, origin }) {
  const truth = await resolveProductionState({ cwd, projectKey, origin });
  return {
    ok: !truth.blockDeploy,
    drift: truth.blockDeploy,
    deployPercent: truth.deploy?.percent,
    missingMarkers: truth.deploy?.routeResults?.["/pricing"]?.requiredMissing || [],
  };
}
