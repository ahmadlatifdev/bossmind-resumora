...
function loadConfig(cwd = process.cwd()) {
  const p = path.join(cwd, "config/bossmind-stripe-session-recovery.json");
...
function isolatedProfileDir(cwd) {
  ...
  return path.join(cwd, config.isolatedProfile?.relativeDir || "windows-heal/stripe-runtime/chrome-user-data");
}
...
function scanProfileArtifacts(profileDir, cwd) {
  try {
    const script = path.join(cwd, "scripts/bossmind-stripe-profile-scan.mjs");
    const r = spawnSync("node", [script, `--profile-dir=${profileDir}`], {
      encoding: "utf8",
...
function runPowerShellRepair(cwd, { apply = false, profile = "", closeChrome = false, skipIsolatedLaunch = false } = {}) {
  const ps1 = path.join(cwd, "scripts", "bossmind-stripe-session-repair.ps1");
...
async function detectStripeSessionIssue({ cwd = process.cwd(), profileName = "" } = {}) {
  const config = loadConfig(cwd);
  ...
  const detection = {
    ...
    primaryCause: null,
    confidence: 0,
    isolatedProfileDir: isolatedProfileDir(cwd).replace(/\\/g, "/"),
  };
  ...
  for (const p of profiles) {
    const scan = !chromeRunning ? scanProfileArtifacts(p.dir, cwd) : null;
    if (scan) {
  ...
  if (edge) {
    const proofDir = path.join(cwd, "windows-heal", "reports", "stripe-recovery", "detect-probe");
...
async function verifyStripeDashboardAccess({ cwd = process.cwd(), saveProof = true } = {}) {
  const config = loadConfig(cwd);
  const edge = edgeExecutable();
...
async function verifyStripeDashboardHealth({ cwd = process.cwd() } = {}) {
...
async function runStripeSessionRecovery({
  ...
  const { config, detection } = await detectStripeSessionIssue({ cwd, profileName: profile });

  let repair = { ok: true, skipped: !apply, note: "Dry-run detect only." };
  if (apply) {
    repair = runPowerShellRepair(cwd, {
      apply: true,
      profile: profile || "",
  ...
  const isolatedProfile = repair.report?.isolatedProfile || {
    dir: isolatedProfileDir(cwd),
    launcher: path.join(cwd, "windows-heal/stripe-runtime/launch-stripe-dashboard.ps1"),
  };

  let verification = { ok: true, skipped: true };
  if (verify) {
    verification = await verifyStripeDashboardAccess({ cwd, saveProof: true });
  }
  ...
  const outDir = path.join(cwd, "windows-heal", "reports");
