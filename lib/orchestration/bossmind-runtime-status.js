...
async function getBossMindRuntimeOverview({ projectKey, neonEnabled }) {
  ...
  const overview = {
    ...
    runtimeSync: {
      localStatusPath: path.join(cwd, ".bossmind", "runtime-sync", "status.json"),
      localStatus: null,
      authority: null,
    },
    neon: {
  ...
  try {
    if (fs.existsSync(overview.runtimeSync.localStatusPath)) {
      overview.runtimeSync.localStatus = JSON.parse(
        fs.readFileSync(overview.runtimeSync.localStatusPath, "utf8")
      );
    }
  } catch {
    overview.runtimeSync.localStatus = null;
  }
  ...
  if (neonEnabled) {
    ...
    if (sql) {
      ...
      overview.runtimeSync.authority = await neon.getRuntimeAuthority({
        projectKey,
        authorityKey: process.env.BOSSMIND_AUTHORITY_KEY || "luxury_ui_baseline",
      });
    }
  }
