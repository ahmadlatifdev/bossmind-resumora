...
async function runRepairFlow({
  ...
  const flowSummary = {
    pipeline:
      "Sentry -> shared error memory -> LangGraph supervisor/worker -> DeepSeek triage (external) -> Codex patch candidate (external/GitHub) -> BossMind validation -> deploy -> verify -> save fix pattern to Neon",
