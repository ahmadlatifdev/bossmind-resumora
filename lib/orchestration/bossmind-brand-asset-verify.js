...
async function probeLiveLogo(origin, expectedHash) {
  const url = `${origin.replace(/\/$/, "")}${BRAND_LOGO_SRC}`;
  const res = await fetch(url, { headers: { "cache-control": "no-cache" } });
  if (!res.ok) return { ok: false, url, status: res.status, hashMatch: false };
  const buf = Buffer.from(await res.arrayBuffer());
  const crypto = require("crypto");
  const hash = crypto.createHash("sha256").update(buf).digest("hex");
  return { ok: true, url, status: res.status, hash, hashMatch: hash === expectedHash };
}
...
async function runBrandAssetVerification({ cwd = process.cwd(), origin = null, probeHtml = true } = {}) {
  ...
  if (origin && probeHtml) {
    try {
      ...
      htmlProbe = {
        url: `${origin}/pricing`,
        status: res.status,
        hasLockedSrc: html.includes(BRAND_LOGO_SRC),
  ...
  const ok =
    ...
    hashOk &&
    conflicts.length === 0 &&
    (!htmlProbe || (htmlProbe.hasLockedSrc && htmlProbe.hasBrandMarker && !htmlProbe.hasLegacySvg)) &&
