...
function scanConflictingLogoSources(cwd) {
  ...
  const allowFiles = new Set([
    ...
    path.join(cwd, "lib/marketing/brand-asset-authority.constants.js").replace(/\\/g, "/"),
  ...
  const dupPaths = [
    ...
    path.join(cwd, "public/brand/resumora-logo-original.png"),
    path.join(cwd, "public/brand/resumora-logo-official.png"),
    path.join(cwd, "public/brand/resumora-logo-official.jpg"),
  ];
...
async function runBrandAssetVerification({ cwd = process.cwd(), origin = null, probeHtml = true } = {}) {
  ...
  const fileOk = fs.existsSync(path.join(/* turbopackIgnore: true */ cwd, BRAND_LOGO_PUBLIC_FILE));
