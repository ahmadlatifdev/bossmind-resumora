...
function satisfiesBrandMarker(html, marker) {
  if (marker.includes("resumora-logo") || marker.includes("data-rs-brand-logo")) {
    return (
      html.includes("/brand/resumora-logo-official-transparent") ||
      html.includes("/brand/resumora-logo-official.jpg") ||
...
async function probePricingPage(origin, manifest) {
  ...
  const requiredMissing = required.filter((m) => !satisfiesBrandMarker(html, m.replace(/\\"/g, '"')));
  ...
  return {
    ...
    hasLogo:
      html.includes("/brand/resumora-logo-official-transparent") ||
      html.includes("/brand/resumora-logo-official.jpg") ||
      html.includes("data-rs-brand-logo") ||
      (html.includes("resumora-logo.png") && !html.includes("resumora-logo.svg")),
  };
}
