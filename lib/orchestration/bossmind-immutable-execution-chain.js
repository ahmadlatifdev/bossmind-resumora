...
async function probePricingPage(origin, manifest) {
  ...
  const studioTiers = (html.match(/Studio tiers/gi) || []).length;
  const paliersStudio = (html.match(/Paliers studio/gi) || []).length;
  const headingCount = Math.max(studioTiers, paliersStudio);
  ...
  return {
    ...
    hasLogo: html.includes("resumora-logo"),
  };
}
