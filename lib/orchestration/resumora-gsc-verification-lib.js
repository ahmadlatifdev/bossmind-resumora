...
async function pingGoogleSitemap(sitemapUrl) {
  ...
  try {
    const res = await fetch(url, {
      method: "GET",
      headers: { "user-agent": "ResumoraGscSitemapPing/1.0 (+https://resumora.net)" },
    });
    const text = (await res.text()).slice(0, 500);
    return { ok: res.ok, status: res.status, url, bodyPreview: text };
  } catch (e) {
    return { ok: false, error: e.message || String(e), url };
  }
}

async function submitSitemapToSearchConsole(domain, feedpath = "/sitemap.xml") {
  const tok = await getGoogleAccessToken();
  if (!tok.ok) return { skipped: true, reason: tok.reason };

  const siteUrl = `sc-domain:${domain.replace(/^www\./, "")}`;
  const pathEnc = encodeURIComponent(feedpath.startsWith("/") ? feedpath : `/${feedpath}`);
  const apiUrl = `https://www.googleapis.com/webmasters/v3/sites/${encodeURIComponent(siteUrl)}/sitemaps/${pathEnc}`;
  ...
  try {
    ...
    const json = await res.json().catch(() => ({}));
    if (!res.ok) {
      return { ok: false, status: res.status, error: json.error?.message || res.statusText, siteUrl, feedpath };
    }
    return { ok: true, siteUrl, feedpath, submitted: true };
  } catch (e) {
    return { ok: false, error: e.message || String(e), siteUrl, feedpath };
  }
}

async function fetchSitemapHealth(origin) {
  ...
  const [robots, sitemap] = await Promise.all([
    fetch(robotsUrl, {
      headers: { "user-agent": "ResumoraSitemapHealth/1.0" },
    }).then(async (r) => ({ ok: r.ok, status: r.status, text: (await r.text()).slice(0, 4000) })),
    fetch(sitemapUrl, {
      headers: { "user-agent": "ResumoraSitemapHealth/1.0" },
    }).then(async (r) => ({ ok: r.ok, status: r.status, text: (await r.text()).slice(0, 8000) })),
  ]);

  const robotsOk = robots.ok && /sitemap/i.test(robots.text || "");
  const sitemapOk =
    sitemap.ok &&
  ...
  return {
    ...
    sitemapUrlCount: ((sitemap.text || "").match(/<loc>/gi) || []).length,
  };
}
