...
const { loadProjectEnv } = require("../shared/load-project-env");
...
function scanResumoraLocal(projectRoot) {
  ...
  results.password_reset = async () => {
    const routes = [
      "pages/api/engagement/password-reset/request.js",
      "pages/api/engagement/password-reset/verify.js",
      "pages/api/engagement/password-reset/complete.js",
      "pages/forgot-password.js",
      "pages/reset-password.js",
    ];
    const missing = routes.filter((r) => !fs.existsSync(path.join(root, r)));
    if (missing.length) return { status: STATUS.MISSING, detail: `routes_${missing.length}` };
    try {
      const { auditPasswordResetHealth } = require("../engagement/password-reset");
      const h = await auditPasswordResetHealth();
      const siteCopy = fs.readFileSync(path.join(root, "lib/marketing/site-copy.js"), "utf8");
      const i18n = siteCopy.includes("forgotPasswordTitle") && siteCopy.includes("forgotPasswordSubtitle");
      if (!h.database || !h.tableOk) return { status: STATUS.BROKEN, detail: "schema_or_db" };
      if (!i18n) return { status: STATUS.PARTIAL, detail: "i18n_missing" };
      if (!h.providers.email.configured && !h.providers.devLog) {
        return { status: STATUS.PARTIAL, detail: "delivery_providers_unconfigured", safeFix: "render_env_bundle" };
      }
      return { status: STATUS.ACTIVE, detail: "reset_workflow_ready" };
    } catch (e) {
      return { status: STATUS.BROKEN, detail: e.message || "reset_audit_failed" };
    }
  };
...
async function runSafeFix(fixId, policy) {
  ...
  const script = path.join(/* turbopackIgnore: true */ process.cwd(), "scripts", fix.script);
