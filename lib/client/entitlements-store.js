  ...
  if (stripeSessionId) {
    ...
    if (bySession?.length) {
      const rows = await sql.query(
        ...
        [profileId, planId, stripeSessionId, email || null, metadata, bySession[0].id]
      );
  ...
  if (existing?.length) {
    const rows = await sql.query(
      `UPDATE client_entitlements SET
       ...
       WHERE id = $5
       RETURNING id, profile_id, plan_id, granted_at`,
      [profileId, stripeSessionId, email || null, metadata, existing[0].id]
    );
    return { ok: true, entitlement: rows?.[0], updated: true };
  }
