/**
 * Single-call post-checkout bootstrap — delegates to activation pipeline.
 */
const { runCheckoutActivationPipeline } = require("./checkout-activation-pipeline");
const {
  buildLuxuryStages,
...
async function bootstrapClientCheckout(actor, { sessionId = null, lang = "en" } = {}) {
  const pipeline = await runCheckoutActivationPipeline(actor, { sessionId, lang });
  ...
  return {
    ...
    activationSteps: pipeline.steps || [],
