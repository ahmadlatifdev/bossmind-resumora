...
const { getFreeEditsCount, freeEditsLabel } = require("./plan-policy");
...
function getDeliverableForPlan(planId, lang = "en") {
  ...
  const freeEdits = plan.freeEdits ?? getFreeEditsCount(planId);
  return {
    planId,
    ...
    freeEdits,
    freeEditsLabel: freeEditsLabel(planId, L),
