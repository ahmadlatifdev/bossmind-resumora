...
async function activateFromStripeSession(stripeSession, { profileId = null, profileEmail = null, lang = "en" } = {}) {
  ...
  const planId = await resolvePlanIdFromSession(stripeSession, stripe);
  if (!planId) {
  ...
  const plans = profileId || pid ? await listEntitlementsForUser(pid || profileId, accountEmail || stripeEmail) : [];
  ...
  const activationFlags = {
    ...
    interviewToolkitReady: planId === "essential_advanced" && plans.length > 0,
  };
  const luxuryStages = buildLuxuryStages(activationFlags, 0, lang);
  const hasAccess = plans.length > 0 || (grantResult?.ok === true && !profileId);
  return {
    ok: grantResult?.ok === true && (profileId ? plans.length > 0 : true),
    planId,
    planActivated: grantResult?.ok === true,
