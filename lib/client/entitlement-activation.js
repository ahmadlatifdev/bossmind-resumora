...
async function finalizeEntitlementBinding(profileId, profileEmail, stripeSession) {
  ...
  if (!profileId) return [];
  if (stripeSession?.id) await linkEntitlementByStripeSession(profileId, stripeSession.id);
  ...
  return listEntitlementsForUser(profileId, stripeEmail || accountEmail);
}
...
async function activateFromStripeSession(stripeSession, { profileId = null, profileEmail = null, lang = "en" } = {}) {
  ...
  let plans =
    profileId || pid
      ? await finalizeEntitlementBinding(pid || profileId, accountEmail || profileEmail, stripeSession)
      : [];
  if (!plans.length && (profileId || pid)) {
...
async function activateBySessionId(sessionId, actor = {}, lang = "en") {
  ...
  if (actor.profileId && session.payment_status === "paid") {
    const plans = await finalizeEntitlementBinding(actor.profileId, actor.profileEmail, session);
    if (plans.length) {
      result.plans = plans;
      result.plansCount = plans.length;
      result.ok = true;
      result.activation = {
        ...(result.activation || {}),
        workspaceReady: true,
      ...
      result.luxuryStages = buildLuxuryStages(result.activation, 0, lang);
      result.conciergeMessage = activeConciergeMessage(result.luxuryStages, lang);
