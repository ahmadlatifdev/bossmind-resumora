...
function presignEnabled() {
  return isS3Enabled() && String(process.env.S3_PRESIGN_ENABLED || "true") !== "false";
}

async function createPresignedReadUrl(storageRef, { expiresIn = 300, disposition = "inline" } = {}) {
  if (!presignEnabled() || !isS3Ref(storageRef)) {
    return { ok: false, reason: "presign_unavailable" };
  }
  const key = decodeS3Ref(storageRef);
  const { bucket } = s3Config();
  try {
    ...
    logStorageDiag("presign_ok", { key, expiresIn });
    return { ok: true, url, expiresIn };
  } catch (e) {
    logStorageDiag("presign_failed", { key, message: e.message });
