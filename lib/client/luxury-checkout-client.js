...
export const LUXURY_CHECKOUT_TIMEOUT_MS = 20000;
export const LUXURY_CHECKOUT_MIN_PREPARE_MS = 5000;
...
export async function runLuxuryCheckoutActivation(sessionId, lang, { signal: outerSignal } = {}) {
  ...
  return { status: "recovery", data: last.data, attempts };
}

function isActivationComplete(data) {
  return data?.activationStatus === "complete" && data?.hasAccess === true;
}
