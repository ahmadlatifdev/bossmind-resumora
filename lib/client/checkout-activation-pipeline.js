...
const { linkEntitlementByStripeSession } = require("./entitlement-activation");
 ...
 */
async function tryCompleteFromStoredEntitlement(actor, sessionId, lang, steps, { source = "database" } = {}) {
  ...
  if (actor?.profileId) {
    ...
    if (!plans.length) plans = minimalPlan(planId, lang);

    steps.push(step("stripe_session", true, { source, cached: true }));
    steps.push(step("payment_confirmed", true, { source }));
    steps.push(step("entitlement_grant", true, { planId, source }));
    steps.push(step("workspace_unlock", true, { plansCount: plans.length, source }));
  ...
  steps.push(step("stripe_session", true, { source, needsSignIn: true }));
  return {
    ok: true,
...
async function runCheckoutActivationPipeline(actor, { sessionId, lang = "en" } = {}) {
  ...
  const cached = await tryCompleteFromStoredEntitlement(actor, sid, lang, steps, { source: "database_precheck" });
  if (cached) {
    cached.elapsedMs = Date.now() - startedAt;
    return cached;
  }
  ...
  if (!stripe) {
    ...
    const fallback = await tryCompleteFromStoredEntitlement(actor, sid, lang, steps, {
      source: "database_stripe_unconfigured",
    });
    if (fallback) {
  ...
  try {
    ...
    const fallback = await tryCompleteFromStoredEntitlement(actor, sid, lang, steps, {
      source: "database_stripe_retrieve_failed",
    });
    if (fallback) {
      fallback.elapsedMs = Date.now() - startedAt;
      return fallback;
