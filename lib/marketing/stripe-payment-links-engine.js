...
const { paymentLinkAfterCompletion, getStudioCheckoutSuccessUrl } = require("./stripe-checkout-urls");
...
function serviceMetadata(service, catalog) {
  const planIds = service.planIds || [];
  const primaryPlan = planIds.find((id) =>
    ["basic", "professional", "elite", "essential_advanced"].includes(id)
  );
  return {
    brand_name: catalog.officialBrand,
    service_key: service.serviceKey,
    bossmind_service_key: service.serviceKey,
    plan_id: primaryPlan || planIds[0] || "",
...
async function persistToNeon(projectKey, manifest) {
  try {
    ...
    return { rows: 0, persisted: false, neonError: e.message };
  }
  ...
  return { rows: rows.filter(Boolean).length };
    ...
    return { rows: rows.filter(Boolean).length, neonError: e.message };
  }
}

/** Sync after_completion redirect on every known payment link (no Neon required). */
async function syncPaymentLinkRedirects({ cwd = process.cwd(), force = false } = {}) {
  const catalog = loadCatalog(cwd);
  const { stripe, reason } = createStripeServerClient();
  if (!stripe) return { ok: false, error: "stripe_unconfigured", reason, synced: [] };
  ...
  if (fs.existsSync(lockPath)) {
    try {
      ...
      if (Array.isArray(lock.services) && lock.services.length) services = lock.services;
    } catch {
      /* use catalog */
    }
  }
  ...
  for (const service of services) {
    const sk = service.serviceKey || service.service_key;
    if (!sk) continue;
    let link = null;
    if (service.paymentLinkId) {
      try {
        link = await stripe.paymentLinks.retrieve(service.paymentLinkId);
      } catch {
        link = null;
      }
    }
    if (!link) link = await findPaymentLinkForService(stripe, sk);
    if (!link?.id) {
      synced.push({ serviceKey: sk, ok: false, reason: "link_not_found" });
      continue;
    }
    try {
      const updated = await stripe.paymentLinks.update(link.id, {
        after_completion: afterCompletion,
        ...(force && link.active ? { active: true } : {}),
      });
      const redirectUrl = updated?.after_completion?.redirect?.url || "";
      synced.push({
        serviceKey: sk,
        ok: redirectUrl.includes("/studio") && redirectUrl.includes("session_id"),
        paymentLinkId: link.id,
        redirectUrl,
      });
    } catch (e) {
      synced.push({ serviceKey: sk, ok: false, error: e.message });
    }
  }
  const ok = synced.length > 0 && synced.every((s) => s.ok);
  return { ok, synced, successUrl: getStudioCheckoutSuccessUrl() };
}
...
async function verifyPaymentLinks({ cwd = process.cwd() } = {}) {
  ...
  const probes = [];
  const expectedRedirect = paymentLinkAfterCompletion().redirect?.url || "";
  for (const s of manifest.services || []) {
    let linkOk = false;
    ...
    try {
      ...
      redirectOk =
        redirectUrl.includes("/studio") &&
        redirectUrl.includes("session_id") &&
        (!expectedRedirect || redirectUrl.startsWith(expectedRedirect.split("?")[0]));
      if (s.paymentLinkUrl) {
  ...
  const ok = probes.every((p) => p.linkOk && p.redirectOk && p.publicAccessible);
