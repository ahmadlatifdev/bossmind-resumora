...
} from "@/lib/marketing/stripe-plan-map";
...
export function useStripeCheckout() {
  ...
  const handleCheckout = async (planId, planName, planPrice) => {
    const resolvedPublicPriceId = resolveStripePriceId(planId);
    if (!resolvedPublicPriceId) {
      logStripeInternal("STRIPE_PRICE_ID_MISSING", {
        planId,
        canonicalEnv: CANONICAL_PRICE_ENV?.[planId],
        note: "Expose a valid price_* via NEXT_PUBLIC_STRIPE_PRICE_* env (see .env.example); restart dev after edits.",
      });
      return;
    }
