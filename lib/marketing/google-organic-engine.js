...
function loadSeoExpansionTargets(root = process.cwd()) {
  try {
    const p = path.join(root, "config/resumora-google-traffic-engine.json");
    const cfg = JSON.parse(fs.readFileSync(p, "utf8"));
    return cfg.seoExpansionTargets || [];
  } catch {
    return [];
  }
}

function buildSeoExpansionBriefs(weekId, root = process.cwd()) {
  return loadSeoExpansionTargets(root).map((t) => ({
    slug: t.slug,
    path: `/solutions/${t.slug}`,
    keywords: t.keywords || [],
    weekId,
    schema: ["Service", "FAQPage", "BreadcrumbList"],
    cta: `${getSiteUrl()}/pricing`,
  }));
}
...
async function generateGoogleOrganicBundle({ weekId = isoWeekId() } = {}) {
  ...
  return {
    ...
    seoExpansionTargets: buildSeoExpansionBriefs(weekId, process.cwd()),
    youtube: {
