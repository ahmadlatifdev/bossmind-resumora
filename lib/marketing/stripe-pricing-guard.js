...
function pricingSetupHintForPlan(planId) {
  ...
  return `Use a one-time Stripe Price (Dashboard: one-time, not recurring). Set ONE valid price_* in .env.local — preferred: ${canonical}=price_xxx — aliases: ${unique.join(", ")}. Restart dev after edits.`;
}
