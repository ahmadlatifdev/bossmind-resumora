...
async function main() {
  ...
  const lockedLogoPath = path.join(pub, "brand", "resumora-logo-official-transparent.png");
  if (!fs.existsSync(lockedLogoPath)) {
    console.error("resumora-brand-icons: missing public/brand/resumora-logo-official-transparent.png (brand authority lock)");
    process.exit(1);
  }
  const logoPngBuf = await sharp(lockedLogoPath).png().toBuffer();

  const ogLogo = await sharp(logoPngBuf)
  ...
  console.log(
    "resumora-brand-icons: wrote favicon.ico, favicon-*.png, apple-touch-icon.png, icon-192/512, android-chrome-*, icon.svg, og-resumora-brand.png (logo source: brand/resumora-logo-official-transparent.png); synced sw.js"
  );
}
