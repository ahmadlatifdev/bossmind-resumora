...
const VIDEO_CONTENT_TYPES = new Set([
  ...
  'video/x-msvideo',
  'video/x-matroska',
  'application/octet-stream', // allow mp4 uploaded without contentType
]);
...
function descriptionFromEnv(filePath) {
  ...
  let text = String(tmpl)
    ...
    .slice(0, 1800);
  // Prompt creators / ops to use Bilibili AI translation + AI Voice for ZH audiences.
...
function tidFromEnv() {
  // Default partition: 生活 > 日常 (21) — override with BILIBILI_TID
...
/**
 * Upload a local buffer/file stream to Bilibili UPOS then submit.
 * @param {Buffer} fileBuffer
 * @param {{ fileName: string, title: string, description: string, cookies: object }} opts
 */
async function uploadBufferToBilibili(fileBuffer, opts) {
  ...
  if (!merge.ok || (merge.json && Number(merge.json.OK) !== 1 && merge.json.OK !== undefined)) {
    // Some endpoints return OK:1; others return empty success
    if (!merge.ok) {
  ...
  // filename inside upos for submit (filename without path)
...
/**
 * Download GCS object and publish to Bilibili.
 */
